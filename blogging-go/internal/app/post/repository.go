...
const (
	...
	WHERE pb.DeletedAt IS NULL
	ORDER BY ps.TotalInteraksi DESC, pb.CreatedAt DESC`
...
// GetAllPosts retrieves all posts with basic pagination (no filters/search)
func GetAllPosts(db *sqlx.DB, page, limit int) ([]PostinganBlogListResponse, *PaginationInfo, error) {
	...
	// Apply pagination
	offset := (page - 1) * limit
	query := basePostQuery + fmt.Sprintf(" OFFSET %d ROWS FETCH NEXT %d ROWS ONLY", offset, limit)
	...
	var totalCount int
	err = db.QueryRow(baseCountQuery).Scan(&totalCount)
	if err != nil {
